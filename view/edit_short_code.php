	...
	foreach($data['form_data'] as $key =>$value){
		...
		if($value['field_type']=='checkbox'){
			foreach ($value['checkbox'] as $key => $chechdat) {
				$checkboxData=$checkboxData.'<div class="row"><div class="form-group col-md-10"><label for="text">Checkbox Field Name</label><input type="text" class="form-control checkbox_field_name " value="'.$chechdat.'" id="text" placeholder="Enter text"></div><div class="col-md-2"><button class="Add_more_checkbox btn btn-primary" id="'.$addMoreI.'" type="button">Add</button></div></div><div class="wp_checkbox_Addmore'.$addMoreI.'"></div>';
			}
		...
		if($value['field_type']=='radio'){
			...
			$radioButtonData = $radioButtonData.'<div class="row"><div class="form-group col-md-10"><label for="text">Radio Button Field Name</label><input type="text" class="form-control label_name radio_button_child" id="text" value="'.$raddata.'" placeholder="Enter text" name="label[]"></div><div class="col-md-2"><button class="add_more_radio_button btn btn-primary" id="'.$addMoreI.'" type="button">Add</button></div></div><div class="wp_radio_button_Addmore'.$addMoreI.'"></div>';
			}
		}
		$addMoreI++;
...
<div class="wp_append_checkbox'.$i.'">'.$checkboxData.'</div>
	...
	<form method="post" action="#">
		...
		<div id="output" style="display:none">'.$i.'</div>
